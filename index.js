...
(async () => {
  ...
  console.log("Save this STRING_SESSION:");
  ...
  client.addEventHandler(
    async (event) => {
      try {
        ...
        if (!sourceChannels.includes(chat.username)) return;

        const cleaned = cleanText(message.message);

        if (!cleaned) return;

        await sendNewsToAdmins(cleaned);

        console.log("News sent:", chat.username);
  ...
  console.log("News listener is running...");
